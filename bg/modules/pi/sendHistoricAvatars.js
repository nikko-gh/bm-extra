...
export async function sendHistoricAvatars(steamId, tabId, returnObject) {
    ...
    const resp = await fetch(`https://player-insight.flqyd.dev/api/steam/avatars/${steamId}?token=${key}`);
    ...
    if (resp?.status !== 200) {
        console.error(`Failed to fetch historic steam avatars! | ${steamId} | ${key.substring(0, 10)} | ${resp?.status}`);
        return sendResponse(tabId, returnObject, errorString.failedToFetch);
    }

    const data = await resp.json();
    const value = data.data.avatars;

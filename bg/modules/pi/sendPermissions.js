...
export async function sendPiPermissions(tabId, returnObject) {
    ...
    const resp = await fetch(`https://player-insight.flqyd.dev/api/permissions?token=${key}`);
    if (resp?.status === 403) return sendResponse(tabId, returnObject, errorString.invalidKey);
    if (resp?.status !== 200) {
        console.error(`Failed to fetch PI permissions! | ${key.substring(0, 10)} | ${resp?.status}`);
        return sendResponse(tabId, returnObject, errorString.failedToFetch);
    }

    const data = await resp.json();
    const value = data.data.perms;

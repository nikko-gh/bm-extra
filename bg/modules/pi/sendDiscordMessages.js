...
export async function sendDiscordMessages(path, tabId, returnObject) {
    ...
    const resp = await fetch(`https://player-insight.flqyd.dev/api/discord/messages/${path}?token=${key}`);
    ...
    if (resp?.status !== 200) {
        console.error(`Failed to request messages | Path: ${path}| API KEY: ${key.substring(0, 10)}... | Status: ${resp?.status}`);
        return sendResponse(tabId, returnObject, errorString.failedToFetch);
    }

    const data = await resp.json();
    sendResponse(tabId, returnObject, data.data);
}
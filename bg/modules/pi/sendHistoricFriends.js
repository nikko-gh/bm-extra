...
export async function sendHistoricFriends(steamId, tabId, returnObject) {
    ...
    const resp = await fetch(`https://player-insight.flqyd.dev/api/steam/friends/${steamId}?token=${key}`);
    ...
    if (resp?.status !== 200) {
        console.error(`Failed to fetch historic steam friends! | ${steamId} | ${key.substring(0, 10)} | ${resp?.status}`);

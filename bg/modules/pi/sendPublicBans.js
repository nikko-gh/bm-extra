...
export async function sendPublicBans(steamId, tabId, returnObject) {
    ...
    const resp = await fetch(`https://player-insight.flqyd.dev/api/steam/bans/${steamId}?token=${key}`);
    ...
    if (resp?.status !== 200) {
        console.error(`Failed to fetch public bans! | ${steamId} | ${key.substring(0, 10)} | ${resp?.status}`);
